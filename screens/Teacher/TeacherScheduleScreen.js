...
export default function TeacherScheduleScreen({ navigation, route }) {
    ...
    const loadAttendances = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const res = await api.get(`/teacher/${teacherId}/attendances`);
    ...
    const handleChangeStatus = async (sessionId, newStatus) => {
        ...
        setLoading(true);
        try {
            await api.post("/attendance/change-confirmation-status", { class_session_id: sessionId, teacher_id: teacherId, status: newStatus });
                            ...
                            return (
                                <View key={c.class_session_id} style={tw`bg-white mb-4 rounded-xl p-4 flex-row items-center shadow`}>
                                    ...
                                    <View style={tw`gap-2`}>
                                        {isConfirmed ? (
                                            <>
                                                <TouchableOpacity style={tw`px-3 py-1 bg-gray-600 rounded`} onPress={() => handleChangeStatus(c.class_session_id, 'cancelled')}>
                                                    <Text style={tw`text-white`}>Otkaži</Text>
                                                </TouchableOpacity>
                                                <View style={tw`px-3 py-1 bg-orange-500 rounded`}>
                                                    <Text style={tw`text-white`}>Potvrđеnо</Text>
                                                </View>
                                            </>
                                        ) : (
                                            <TouchableOpacity style={tw`px-3 py-1 bg-[#112E50] rounded`} onPress={() => handleChangeStatus(c.class_session_id, 'confirmed')}>
                                                <Text style={tw`text-white`}>Potvrdi</Text>
                                            </TouchableOpacity>
                                        )}
                                        {/* Novo dugme za otvaranje liste učenika */}
                                        <TouchableOpacity
                                            style={tw`px-3 py-1 bg-blue-500 rounded`}
                                            onPress={() => navigation.navigate('SessionStudents', { sessionId: c.class_session_id })}
                                        >
                                            <Text style={tw`text-white`}>Učenici</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            );
